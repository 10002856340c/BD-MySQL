CREATE DEFINER=`root`@`localhost` PROCEDURE `diferencia_porcentual2`(vfecha1 DATE , vfecha2 DATE)
BEGIN
...
-- aplicamos el CASE poniendo condiciones 
CASE 
   WHEN vvariacion  > 10 THEN SET vmensaje = 'Verde';
   WHEN vvariacion BETWEEN -10 AND 10 THEN SET vmensaje = 'Amarillo';
    WHEN vvariacion < -10 THEN SET vmensaje = 'Rojo';
END case;

...
SELECT A.NOMBRE_DEL_PRODUCTO, B.PRECIO AS TOTAL FROM tabla_de_productos A 
INNER JOIN items_facturas B 
ON A.CODIGO_DEL_PRODUCTO = B.CODIGO_DEL_PRODUCTO;

-- Costo 211939.80

SELECT A.NOMBRE_DEL_PRODUCTO, ROUND(SUM(B.PRECIO*B.CANTIDAD),2) AS TOTAL FROM tabla_de_productos A 
INNER JOIN items_facturas B 
ON A.CODIGO_DEL_PRODUCTO = B.CODIGO_DEL_PRODUCTO
